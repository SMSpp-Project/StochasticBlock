...
static void generateWeightedRandomSubset(
    size_t n, size_t k, const std::vector<double> & weights,
    std::vector<ScenarioGenerator::ScenarioIndex> & ind, std::mt19937 & rng )
{
 if ( k > n )
...
static BlockConfig *
generate_default_cfl_config( DiscreteScenarioSet::ScenarioIndex size )
{
...
DiscreteScenarioSet::get_selected_scenarios() const
{
 if ( !is_initialized ) {
  throw std::runtime_error(
      "Pool has not been initialized. Call init_random_pool() or "
...
void DiscreteScenarioSet::init_random_pool( ScenarioIndex pool_size )
{
 ...
 if ( pool_size > 0 ) {
  // Reserve space for pool weights that will be populated by
  // update_pool_weights
    ...
    ScenarioIndex target_pool_size )
{
 ...
 if ( f_solver_config ) {
  ...
  cflBlock->load( n_scenarios,  // Number of facilities
                  ...
                  true,  // unsplittable: each scenario assigned to exactly one
                         // representative
...
DiscreteScenarioSet::CMatrix DiscreteScenarioSet::compute_transport_cost_matrix(
