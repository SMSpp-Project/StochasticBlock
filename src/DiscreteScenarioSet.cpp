...
#ifndef NDEBUG
 std::cout <<
  "DEBUG [extract_scenarios_and_assignments_from_cfl_block]: Starting extraction from CFL block"
  << std::endl;
#endif
...
void DiscreteScenarioSet::set_config( Configuration * config ) {
 ...
 auto * simple_poolSize = dynamic_cast< SimpleConfiguration< int > * >(
  config );
 if( simple_poolSize ) {
 ...
 auto * poolSize_solver_pair = dynamic_cast< SimpleConfiguration< std::pair<
 ...
 auto * config_pair = dynamic_cast< SimpleConfiguration< std::pair<
...
void DiscreteScenarioSet::deserialize( const netCDF::NcGroup & group ) {
 ...
 bool probsLoaded = ::deserialize( group , "poolWeights" , nbScenarios ,
                                   setWeights );
...
void DiscreteScenarioSet::serialize( netCDF::NcGroup & group ) const {
 ...
 auto probsVar = group.addVar( "poolWeights" , netCDF::NcDouble() ,
                               nbScenariosDim );
...
void DiscreteScenarioSet::init_random_pool( ScenarioIndex pool_size ) {
 ...
 if( pool_size > 0 ) {
  ...
  generateWeightedRandomSubset( nbScenarios , pool_size , setWeights ,
                                scenarioIndexes , rng );
...
void DiscreteScenarioSet::init_representative_pool(
 ...
 if( f_solver_config ) {
  ...
  auto [ capacities, fixed_costs, demands ] = create_cfl_problem_data(
   n_scenarios , setWeights );

  // Compute the transport cost matrix with ell-powered distances
  auto transport_costs = compute_transport_cost_matrix(
   n_scenarios , scenario_size , this->ell );
...
#ifndef NDEBUG
  std::cout << "DEBUG [init_representative_pool]: Using solver: " << solver->
   classname() << std::endl;
#endif
...
double DiscreteScenarioSet::compute_scenario_distance(
 const Eigen::VectorXd & scenario1 ,
...
void DiscreteScenarioSet::apply_baseline_selection(
 ...
 for( ScenarioIndex i = 0 ; i < nbScenarios ; ++i ) {
  double weight = ( i < setWeights.size() )
                   ? setWeights[ i ]
                   : 1.0 / nbScenarios;
...
#ifndef NDEBUG
 std::cout << "DEBUG [apply_baseline_selection]: Selected " << scenarioIndexes.
  size()
