 ...
 * therefore they can just program agaist the [MultiStage]ScenarioGenerator
 * interface.
 *
 ...
 * random variables (say, a pair ( c , a ) where c is the vector c[ j ]
 * denoting the cost of a given commodity at a predetermined set of locations
 * j \in J, and a is the vector a[ j ] denoting the availability of the same
 * commodity at the same places; or the matrix of pairs where each row
 * correspond to a different commodity, ...). As far as the base classes are
 * concerned, a scenario x_t for a given stage t is just a vector of
 * appropriate length d_t, whose internal structure is supposed to be fixed
 * and known to the user. Also, the base classes make no assumption on how the
 * variable X_t depends on ( x_0 , ... , x_{t-1} ) (the history at stage t).
 *
 ...
 * value x_0. This is why there are two separate classes: ScenarioGenerator and
 ...
 *   constructs a :ScenarioGenerator of the specific type dictated by the
 *   string argument (using the factory)
 *
 ...
 *   netCDF file with the given filename, passing through
 *   new_ScenarioGenerator( netCDF::NcGroup ) that reads some top-level
 *   information from the group (either the class type or a filename), finally
 *   down to the pure virtual deserialize( netCDF::NcGroup ) that actually
 *   implements the deserialization for the specific :ScenarioGenerator
 *
 ...
 * number of realizations that can be produced. Basically implies the maximum
 * number of them. */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 ...
 static constexpr INFScenario = Inf< ScenarioIndex >();
...
/*--------------------------------------------------------------------------*/
  ...
  * object of type specified by classname (a std::string with the name of the
  * class inside). Note that the method is static because the factory is
...
/*--------------------------------------------------------------------------*/
					      ...
					      const std::string & filename )
 {
  try {
   ...
   return( ScenarioGenerator::deserialize( f ) );
   }
  catch( netCDF::exceptions::NcException & e ) {
   std::cerr << "netCDF error " << e.what() << " in deserialize"
	     << std::endl;
   }
...
/*--------------------------------------------------------------------------*/
  ...
  * where it clearly already has, plus whatever other information is required
  * by the specific :ScenarioGenerator.
  *
 ...
 * in mind (and a different cost). Once a scenario pool is initialised it can
 * be traversed by going through the scenarios in the pool one by one; at each
 ...
 * pool from one another, which means that there is no assumption on the order
 * in which the scenarios of the pool is visited. Also, there should be no
 * need to re-visit a scenario once the user has been visited it and moved on,
 * so "the clock of the pool only moves forward" (if the user needs to look at
 * more than one scenario at the time they will have to save them in their own
 * data structure.
 *  @{ */
  ...
  * than one generator, if this is the case the method may have to be changed
  * (but it is not impossible to use the same seed for all of them, or to use
  * the seed within a random number generator that is used to generate the
  * seeds, so just one seed may well be enough). */
...
/*--------------------------------------------------------------------------*/
  ...
  * optimization model that has only "seen" a (much) smaller number of
  * scenarios [see init_representative_pool()].
  *
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  * Transport Problem need be solved. Thus, this method may have a nontrivial
  * computational cost. Then, the decisions taken by such model can be
  * evaluated with a simulation on a (much) larger number of "uniformly
 ...
 * in time, to the the general framework of multistage problem where a number
 * T > 0 of time instant (stages) are defined and a scenario ( x_0 , ... ,
 * x_{T-1} ) is a realization of a finite horizon stochastic process ( X_0 ,
 * ... , X_{T-1} ), where for each 0 <= t <= T - 1, the random variable X_t
 * has values in some euclidean space R^(d_t). The base class makes no
 * assumption on the "internal structure" of x_t, which is just a vector of
 * appropriate length d_t, whose internal structure is supposed to be fixed
 * and known to the user.
 *
 * In the following, at a stage 0 < t < T, we denote by H_t = ( X_0 = x0 ,
 * ... , X_{t-1} = x_{t-1} ) the history of the process up to time t (H_0
 * being obviously empty). In general, in order to compute a scenario ( x_0 ,
 * ... , x_{T-1} ) from the input stochastic process ( X_0 , ... , X_{T-1} ),
 * the class will need to be able to draw for every t < T from the random
 * variable X_t | H_t. Yet, the class makes no specific assumption on how the
 * variable X_t depends on H_t. For instance, the case where each stage is a
 * random variable X_t independent from the past history H_t (i.e.,
 * X_t | H_t = X_t) is one case, where a sensible implementation is just as a
 * vector of ScenarioGenerator *, one for each of the stages. Other
 ...
 * depend on the entire history. The interfaces strives to avoid to
 * distinguish those cases by having each stage x_t generated / read only
 * when its history H_t is clearly specified, irrespectively on what the
 * dependency is.
 *
 * Basically, the class provide means for moving along (back and forth) the
 ...
 * in the single-stage case, which is why a large part of the interface can be
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 ...
 static constexpr INFStage = Inf< StageIndex >();
 ...
 * current stage is initialised to t = 0 (first stage, with empty H_t) when
 ...
 * that give the same size. Thus, in this case one could have
 * get_support_size() to return s_t for t = get_current_stage(), similarly to
 *  get_scenario_size(), and even specify the size of each sub-pool. However,
 * there does not seem to ba a general way of doing this for all reasonable
 * distributions that actually depend on H_t, in that the right number of
 * branching in the scenario tree at a node may depend on the whole path to
 * the root. The design choice is therefore that these more detailed
 * information about how the scenario tree is organized are depending on the
 * specific derived class of MultiStageScenarioGenerator and read together
 * with its description in deserialize(); hence, they cannot be changed on the
 * fly. If this turns out to be a problem than some Configuration will have to
 * be used, but we'll cross that bridge when (if) we'll come to it.
 *
