...
/*--------------------------------------------------------------------------*/
 ...
 /** In the context of scenario reduction, one may want to construct a 
  * problem-dependent metric to guide the scenario reduction process. In such
  * case, the problem's data need to be extracted and often one may construct
  * tailored problem-dependent metric that uses it. 
  * 
  * ScenarioGenerator provides a virtual function `set_Block` that by default 
  * does nothing.
  * 
  ...
  * the given Block pointer is indeed the one is expected *and* that
  * scenarios are as intended. That is, the scenarios that are at 
  * ScenarioGenerator's (or its derived class) disposal should be coherent 
  * with the expected stochasticity of the Block's problem. 
  * 
  * For instance, a CapacitatedFacilityLocationBlock might have stochasticity 
  * in either demands or capacities. The function `set_Block` takes care of
  * checking that the scenarios generated by the derived class of 
  * ScenarioGenerator are coherent with demands or capacities.
  */
